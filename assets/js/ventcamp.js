...
Ventcamp = {
  ...
  checkMobile: function () {
    ...
    if (
      ...
      $(window).width() < this.options.mobileMenuMaxWidth
    ) {
  ...
  initAnimations: function () {
    ...
    if (
      $(".counter-block")
        ...
        .not(".countdown .counter-block").length
    ) {
      $(".counter-block")
        .not(".animated")
        .not(".animated .counter-block")
    ...
    $(".ytp-player-background").each(function () {
      ...
      if (!_this.mobileDevice && typeof $.fn.YTPlayer == "function") {
        ...
        if (typeof $.fn.YTPlayer != "function") {
          _this.log(
            "Can't find jQuery.YTPlayer function. Video background blocks doesn't work."
          );
        } else {
  ...
  makePseudoSelect: function (i, el) {
    ...
    var closePseudoSelect = function () {
      ...
      selectItem = function ($li) {
        ...
        if (elemPosition + elHeight > dropdownHeight) {
          $dropdown.scrollTop(
            elHeight + elemPosition - dropdownHeight + scrollTop
          );
    ...
    $dropdown.on(
      "mousedown.pseudoSelect click.pseudoSelect",
      "li",
      function (event) {
        event.preventDefault();
          ...
          $.ajax({
            ...
            success: function (msg) {
              ...
              if (msg.indexOf("Success") != -1) {
                $responseBlock.html(
                  '<span class="success-message">Success! You are now subscribed to our newsletter!</span>'
                );
              } else {
                $responseBlock.html(
      ...
      $(".map").each(function () {
        ...
        function createMarkers(place, status) {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            ...
            if (place.international_phone_number) {
              innerContent +=
                '<p class="phone">' + place.international_phone_number + "</p>";
            }
        ...
        function centerMap(map, offsetX, offsetY, relative) {
          var offsetX = typeof offsetX == "number" ? offsetX : 0,
            ...
            point2 = new google.maps.Point(offsetX / scale, offsetY / scale),
        ...
        function successCallback(data) {
          if (typeof data == "object") {
            ...
            if (data.markerImagePath && typeof data.markerImagePath == "string")
              icon = data.markerImagePath;

            if (data.markerAddress && typeof data.markerAddress == "string")
              address = data.markerAddress;
            else if (data.markerLatLng && typeof data.markerLatLng == "object")
        ...
        if ($(this).data("settings")) {
          $.getJSON($(this).data("settings"), successCallback).fail(
            failCallback
          );
        } else {
          failCallback();
  ...
  styleSwitcherColorHandler: function (event, $el) {
    ...
    $("head").append(
      '<link class="style-switcher-css" rel="stylesheet" type="text/css" href="assets/css/colors/' +
        colorName +
        '.css">'
    );
  },
  ...
  //     $(".preloader-mask").delay(100).fadeOut(100);
